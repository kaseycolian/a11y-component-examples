...
| --- | --- | --- |
| `<dialog>` | — | Already `role="dialog"`, and already modal once `showModal()` runs. |
| `<dialog>` | `aria-labelledby` | Points at the visible `<h2>`. The dialog's name. |
| `<dialog>` | `role="alertdialog"` | Example 3 only, and then `aria-describedby` is **required** as well. |
| `<dialog>` | `data-ac-focus` | A selector for what takes focus on open. Defaults to the dialog itself. |
| a `<button>` inside | a real accessible name | The visible way out. Touch has no <kbd>Esc</kbd> key. |
...
Open it with `showModal()`, never by setting the `open` attribute. The attribute gives a non-modal
dialog in the normal flow, with no backdrop and no trap.
...
**Keys deliberately not bound.** All three. Not one of them is this component's, which is the argument
for the native element in one line.

<kbd>Enter</kbd> is the reason example 3 exists: it is the key people press to make a dialog go away,
so whatever sits under it is the answer they will give without reading.
...
| --- | --- | --- |
| open | The `open` attribute the browser sets, the `::backdrop`, and the rest of the page going inert. | `aria-expanded` on the trigger. There is nobody out there to hear it. |
| closed | The element leaving the top layer, and focus back on the opener. | Anything announced. Closing a dialog says nothing — see point 3 below. |
| destructive | The word in the button's own label, plus the accent. | The accent alone (SC 1.4.1), and never by placing it under <kbd>Enter</kbd>. |
| page behind | Scroll-locked by this component, inert by the browser. | — |

Under `forced-colors: active` the backdrop tint and the danger accent are dropped, so the `[FORCED]`
block rebuilds the dialog's border and the button outlines from system colors. The labels are what
...
Focusing the dialog is the default here because it is right more often than not.
...
- **A modal for something that could sit beside the work.** It takes the page away: everything behind
  it is inert, and on a phone it is the whole screen. That is the right trade for a confirmation with
  consequences and for a short form that interrupts a task, and the wrong one for anything the person
...
- [Drawer](../drawer/) — the same modality anchored to an edge of the viewport, and it can also be
  non-modal, which is why it uses `popover` rather than `<dialog>`.
- [Disclosure](../disclosure/) — the non-modal answer: content that opens in place, with the page still

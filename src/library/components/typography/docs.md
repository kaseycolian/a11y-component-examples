...
Your framework or design system almost certainly has a type scale already. Use it, and check it
against the four things on this page, because a hand-rolled scale usually misses at least one: **is
the size in `rem`**, **does muted text still clear 4.5:1**, **is a link identified by something other
than its color**, and **does a box of text survive the reader doubling its spacing**. There is no
ARIA here and no JavaScript. This is enough for a person or an agent to start from.

Each example is separately copyable, but the scale itself is not: take the `[CORE]` section of
`component.css` whole. There is no useful half of a type scale.

## The contract

Eight classes, and none of them mean anything.
...
the document needs and then pick the class that looks right. **SC 2.4.6** is about headings
describing their section; that only helps if the heading is a heading.

Example 2 has both lines on the page at once, with the heading list printed underneath. The spec
asserts that printed list against the real accessibility tree.
...
1.5, and `overflow: hidden` removes the last line with no scrollbar, no error, and nothing
...
`line-height: 1.6` on body copy, as this scale does, means the first of the four changes nothing here
— the reader's setting is already met.
...
A catalogue number, a URL or a German compound noun at `h1` size is wider than 320px, and an unbroken
word does not wrap. Without it, one long word scrolls the whole page sideways.
...
`<h4 class="ac-t-h2">` announces as "heading level 4", and `<div class="ac-t-h2">` announces as its
...
Type survives Windows High Contrast better than most things — every color is replaced with the
reader's own and the text stays readable. Two exceptions matter here.

`opacity` is **not** dropped, so the faint paragraph in example 3 is still faint against the reader's
own background. It is the one failure on this page that high contrast mode does not rescue, and
another reason to set a color instead.

Link color is replaced by `LinkText`, which takes the color cue away from **both** links in example 4
at once. Only the underlined one is still identifiable. The argument draws itself.

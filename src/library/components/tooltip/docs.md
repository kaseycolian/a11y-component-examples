...
| bubble | `role="tooltip"` | The role exists so assistive tech treats it as a description, not a region. |
...
| something the user must know to succeed | visible help text (example 4) |
| longer, and wanted only sometimes | a toggletip (example 3) or a [Disclosure](../disclosure/) |
...
| Key | What it does |
| --- | --- |
| <kbd>Tab</kbd> | Moves to the trigger, which shows the bubble. One stop — the bubble is never one. |
...
tooltip at the person who clicked it.

<kbd>Esc</kbd> is consumed in the capture phase so an enclosing dropdown or drawer does not close on
the same keystroke. A native `<dialog>` is the exception, since its Escape handling is the UA's own
`cancel` event; call `preventDefault()` on that while a tooltip is open if you need the dialog to stay
put.

**Keys deliberately not bound.** Everything else. The bubble holds no controls, so there is nothing in
it to move between — which is the same reason it is not a tab stop.

## States

| State | Signaled by | Never signaled by |
| --- | --- | --- |
| hidden | `hidden` on the bubble. The description is still on the trigger and still announced. | Removing `aria-describedby`. That takes the text from the people who rely on it. |
| shown | The bubble losing `hidden`, positioned against its trigger. | A timer. It stays until you leave, blur, or press <kbd>Esc</kbd>. |
| dismissed | <kbd>Esc</kbd>, remembered until the pointer leaves or focus moves. | Nothing visible — that is the point. Re-showing under an unmoved pointer would make <kbd>Esc</kbd> useless. |

Under `forced-colors: active` the bubble's fill and shadow are dropped, so the `[FORCED]` block gives
it a system border and repaints the arrow. The bubble is still a bubble; only the palette changes.

### Where the bubble goes when it is shown
...
Expected: `"Export report, button"` then the description, on focus, from the still-hidden bubble
(example 1); `"Print invoice, button"` in example 2, with the bubble supplying the name and no second
announcement when it appears; and in example 3, the button's name followed by the answer as a polite
announcement after the click.

The combination to distrust is touch. TalkBack and iOS VoiceOver have no hover, so assume the visible
bubble never appears there and make sure its text is not the only copy of anything.
...
```html
<button type="button">What counts as a seat?</button>
...
const g = AC.createToggletip(el);   // example 3, same shape
```
...
- **`pointer-events: none` on the bubble.** Fails hoverable. If the bubble is eating clicks, move it
  instead.
- **The native `title` attribute.** Example 5, live. See below.
...
Example 5 keeps a native `title` on the page for comparison. It:
...
- [Disclosure](../disclosure/) — the pattern when the content is a region and belongs in the tab order.
- [Icon Button](../icon-button/) — the other way to name a control whose only content is a glyph.
- [Form Field](../field/) and [Text Input](../text-input/) — `.ac-tooltip-btn` and `.ac-tooltip-field*`
  are local copies so this file stands alone; those components and `button` own the canonical versions.

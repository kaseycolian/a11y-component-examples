...
Hand-written source for `skill/pitfalls.md`. `scripts/build-agent-surfaces.mjs` splits this file on
...
named, and `components/<slug>.md` is the next hop.
...
`testing.md` for what does work. `result-panel` is the precedent.

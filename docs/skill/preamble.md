...
most pressure: it is compressed to the claims alone, and `skill/conventions.md` now holds every
...
still worth the read, and do not sync the list to `skill/index.md`.
...
`calc(var(--ac-motion, var(--motion, 1)) * …)` on every duration. `skill/conventions.md` says what
...
comes from. That is this package and the component's own files, for everyone.
